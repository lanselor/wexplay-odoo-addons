...
class RepairOrder(models.Model):
    ...
    def _get_portal_image_records(self):
        self.ensure_one()
        self._check_portal_related_read_access()
        if "x_image_ids" not in self._fields:
            return self.env["wex.image.record"]
        return self.sudo().x_image_ids.sorted(lambda image: (image.sequence, image.id))

    def _get_portal_image_values(self):
        self.ensure_one()
        values = []
        for image in self._get_portal_image_records():
            values.append(
                {
                    "id": image.id,
                    "name": image.name or _("SAT image"),
                    "description": image.description or "",
                    "tags": image.tag_ids.mapped("name")[:4],
                    "has_binary": bool(image.dms_file_id and image.dms_file_id.content),
                    "image_url": f"/my/repairs/{self.id}/images/{image.id}",
                }
            )
        return values
